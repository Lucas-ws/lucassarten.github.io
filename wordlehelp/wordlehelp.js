const words = [
  // word map of worlde words. f calculated based on relative letter frequency to give each word an overall score.
  { word: "alter", f: "4559" },
  { word: "alert", f: "4559" },
  { word: "later", f: "4559" },
  { word: "arose", f: "4534" },
  { word: "irate", f: "4511" },
  { word: "stare", f: "4509" },
  { word: "arise", f: "4451" },
  { word: "raise", f: "4451" },
  { word: "learn", f: "4405" },
  { word: "renal", f: "4405" },
  { word: "saner", f: "4355" },
  { word: "snare", f: "4355" },
  { word: "least", f: "4329" },
  { word: "stale", f: "4329" },
  { word: "steal", f: "4329" },
  { word: "slate", f: "4329" },
  { word: "react", f: "4317" },
  { word: "cater", f: "4317" },
  { word: "crate", f: "4317" },
  { word: "trace", f: "4317" },
  { word: "clear", f: "4307" },
  { word: "store", f: "4284" },
  { word: "loser", f: "4274" },
  { word: "aisle", f: "4271" },
  { word: "atone", f: "4270" },
  { word: "teary", f: "4265" },
  { word: "alone", f: "4260" },
  { word: "adore", f: "4258" },
  { word: "scare", f: "4257" },
  { word: "layer", f: "4255" },
  { word: "relay", f: "4255" },
  { word: "early", f: "4255" },
  { word: "leant", f: "4235" },
  { word: "trade", f: "4233" },
  { word: "tread", f: "4233" },
  { word: "opera", f: "4232" },
  { word: "heart", f: "4229" },
  { word: "hater", f: "4229" },
  { word: "earth", f: "4229" },
  { word: "taper", f: "4207" },
  { word: "pearl", f: "4197" },
  { word: "paler", f: "4197" },
  { word: "tenor", f: "4190" },
  { word: "alien", f: "4177" },
  { word: "aider", f: "4175" },
  { word: "shear", f: "4169" },
  { word: "share", f: "4169" },
  { word: "crane", f: "4163" },
  { word: "tamer", f: "4156" },
  { word: "grate", f: "4151" },
  { word: "great", f: "4151" },
  { word: "spear", f: "4147" },
  { word: "parse", f: "4147" },
  { word: "spare", f: "4147" },
  { word: "realm", f: "4146" },
  { word: "large", f: "4141" },
  { word: "lager", f: "4141" },
  { word: "regal", f: "4141" },
  { word: "glare", f: "4141" },
  { word: "cleat", f: "4137" },
  { word: "eclat", f: "4137" },
  { word: "snore", f: "4130" },
  { word: "baler", f: "4111" },
  { word: "yearn", f: "4111" },
  { word: "blare", f: "4111" },
  { word: "inter", f: "4107" },
  { word: "inert", f: "4107" },
  { word: "stern", f: "4105" },
  { word: "stole", f: "4104" },
  { word: "liner", f: "4097" },
  { word: "smear", f: "4096" },
  { word: "caste", f: "4087" },
  { word: "outer", f: "4082" },
  { word: "route", f: "4082" },
  { word: "scale", f: "4077" },
  { word: "saute", f: "4077" },
  { word: "after", f: "4070" },
  { word: "flare", f: "4060" },
  { word: "feral", f: "4060" },
  { word: "dealt", f: "4053" },
  { word: "delta", f: "4053" },
  { word: "taker", f: "4050" },
  { word: "lathe", f: "4049" },
  { word: "siren", f: "4047" },
  { word: "risen", f: "4047" },
  { word: "rinse", f: "4047" },
  { word: "resin", f: "4047" },
  { word: "yeast", f: "4035" },
  { word: "water", f: "4035" },
  { word: "score", f: "4032" },
  { word: "ratio", f: "4032" },
  { word: "roast", f: "4030" },
  { word: "leapt", f: "4027" },
  { word: "plate", f: "4027" },
  { word: "pleat", f: "4027" },
  { word: "petal", f: "4027" },
  { word: "rouse", f: "4022" },
  { word: "islet", f: "4021" },
  { word: "solar", f: "4020" },
  { word: "ocean", f: "4018" },
  { word: "canoe", f: "4018" },
  { word: "afire", f: "4012" },
  { word: "safer", f: "4010" },
  { word: "trice", f: "4009" },
  { word: "crest", f: "4007" },
  { word: "other", f: "4004" },
  { word: "stead", f: "4003" },
  { word: "ramen", f: "4002" },
  { word: "relic", f: "3999" },
  { word: "haste", f: "3999" },
  { word: "older", f: "3998" },
  { word: "trial", f: "3997" },
  { word: "anger", f: "3997" },
  { word: "range", f: "3997" },
  { word: "trail", f: "3997" },
  { word: "ideal", f: "3995" },
  { word: "enact", f: "3993" },
  { word: "avert", f: "3993" },
  { word: "leash", f: "3989" },
  { word: "shale", f: "3989" },
  { word: "lance", f: "3983" },
  { word: "clean", f: "3983" },
  { word: "trope", f: "3982" },
  { word: "cedar", f: "3981" },
  { word: "paste", f: "3977" },
  { word: "reach", f: "3977" },
  { word: "metal", f: "3976" },
  { word: "swear", f: "3975" },
  { word: "lapse", f: "3967" },
  { word: "onset", f: "3960" },
  { word: "stone", f: "3960" },
  { word: "recap", f: "3955" },
  { word: "caper", f: "3955" },
  { word: "stair", f: "3947" },
  { word: "aside", f: "3945" },
  { word: "horse", f: "3944" },
  { word: "shore", f: "3944" },
  { word: "table", f: "3941" },
  { word: "bleat", f: "3941" },
  { word: "crone", f: "3938" },
  { word: "anode", f: "3934" },
  { word: "metro", f: "3931" },
  { word: "ready", f: "3929" },
  { word: "inlet", f: "3927" },
  { word: "steam", f: "3926" },
  { word: "tried", f: "3925" },
  { word: "poser", f: "3922" },
  { word: "prose", f: "3922" },
  { word: "spore", f: "3922" },
  { word: "their", f: "3921" },
  { word: "stage", f: "3921" },
  { word: "sepia", f: "3919" },
  { word: "email", f: "3918" },
  { word: "idler", f: "3915" },
  { word: "agile", f: "3913" },
  { word: "cream", f: "3904" },
  { word: "payer", f: "3903" },
  { word: "repay", f: "3903" },
  { word: "noise", f: "3902" },
  { word: "tripe", f: "3899" },
  { word: "grace", f: "3899" },
  { word: "laden", f: "3899" },
  { word: "heard", f: "3893" },
  { word: "baste", f: "3891" },
  { word: "beast", f: "3891" },
  { word: "fetal", f: "3890" },
  { word: "argue", f: "3889" },
  { word: "plier", f: "3889" },
  { word: "peril", f: "3889" },
  { word: "acute", f: "3885" },
  { word: "stein", f: "3877" },
  { word: "extra", f: "3877" },
  { word: "sonar", f: "3876" },
  { word: "arson", f: "3876" },
  { word: "panel", f: "3873" },
  { word: "penal", f: "3873" },
  { word: "plane", f: "3873" },
  { word: "drape", f: "3871" },
  { word: "brace", f: "3869" },
  { word: "relax", f: "3867" },
  { word: "shire", f: "3861" },
  { word: "entry", f: "3861" },
  { word: "nicer", f: "3855" },
  { word: "drone", f: "3854" },
  { word: "train", f: "3853" },
  { word: "close", f: "3852" },
  { word: "heron", f: "3850" },
  { word: "sedan", f: "3849" },
  { word: "merit", f: "3848" },
  { word: "ovate", f: "3848" },
  { word: "remit", f: "3848" },
  { word: "timer", f: "3848" },
  { word: "gayer", f: "3847" },
  { word: "ashen", f: "3845" },
  { word: "forte", f: "3845" },
  { word: "urine", f: "3845" },
  { word: "tiger", f: "3843" },
  { word: "nurse", f: "3843" },
  { word: "louse", f: "3842" },
  { word: "snarl", f: "3841" },
  { word: "eater", f: "3840" },
  { word: "feast", f: "3840" },
  { word: "treat", f: "3840" },
  { word: "terra", f: "3840" },
  { word: "spire", f: "3839" },
  { word: "raven", f: "3839" },
  { word: "actor", f: "3838" },
  { word: "sober", f: "3836" },
  { word: "meant", f: "3832" },
  { word: "false", f: "3830" },
  { word: "trend", f: "3829" },
  { word: "coral", f: "3828" },
  { word: "carol", f: "3828" },
  { word: "prone", f: "3828" },
  { word: "agent", f: "3827" },
  { word: "cause", f: "3825" },
  { word: "sauce", f: "3825" },
  { word: "hotel", f: "3824" },
  { word: "skate", f: "3820" },
  { word: "steak", f: "3820" },
  { word: "dream", f: "3820" },
  { word: "stake", f: "3820" },
  { word: "utile", f: "3819" },
  { word: "farce", f: "3818" },
  { word: "angel", f: "3817" },
  { word: "angle", f: "3817" },
  { word: "glean", f: "3817" },
  { word: "harem", f: "3816" },
  { word: "grade", f: "3815" },
  { word: "valet", f: "3813" },
  { word: "tribe", f: "3813" },
  { word: "alike", f: "3812" },
  { word: "cadet", f: "3811" },
  { word: "wrote", f: "3810" },
  { word: "tower", f: "3810" },
  { word: "cheat", f: "3807" },
  { word: "teach", f: "3807" },
  { word: "truce", f: "3805" },
  { word: "recut", f: "3805" },
  { word: "sweat", f: "3805" },
  { word: "waste", f: "3805" },
  { word: "decal", f: "3801" },
  { word: "lower", f: "3800" },
  { word: "creak", f: "3798" },
  { word: "haute", f: "3797" },
  { word: "leach", f: "3797" },
  { word: "ulcer", f: "3795" },
  { word: "cruel", f: "3795" },
  { word: "ultra", f: "3793" },
  { word: "grape", f: "3789" },
  { word: "miser", f: "3788" },
  { word: "bread", f: "3785" },
  { word: "debar", f: "3785" },
  { word: "beard", f: "3785" },
  { word: "rehab", f: "3781" },
  { word: "erase", f: "3780" },
  { word: "royal", f: "3776" },
  { word: "place", f: "3775" },
  { word: "style", f: "3775" },
  { word: "those", f: "3774" },
  { word: "ethos", f: "3774" },
  { word: "anime", f: "3774" },
  { word: "goner", f: "3772" },
  { word: "diner", f: "3771" },
  { word: "slice", f: "3769" },
  { word: "suite", f: "3769" },
  { word: "trove", f: "3768" },
  { word: "overt", f: "3768" },
  { word: "voter", f: "3768" },
  { word: "stave", f: "3763" },
  { word: "refit", f: "3762" },
  { word: "cameo", f: "3759" },
  { word: "lover", f: "3758" },
  { word: "clone", f: "3758" },
  { word: "decor", f: "3756" },
  { word: "talon", f: "3756" },
  { word: "credo", f: "3756" },
  { word: "tonal", f: "3756" },
  { word: "alive", f: "3755" },
  { word: "salve", f: "3753" },
  { word: "slave", f: "3753" },
  { word: "pesto", f: "3752" },
  { word: "filer", f: "3752" },
  { word: "rifle", f: "3752" },
  { word: "chore", f: "3752" },
  { word: "flier", f: "3752" },
  { word: "oaken", f: "3751" },
  { word: "motel", f: "3751" },
  { word: "sower", f: "3750" },
  { word: "worse", f: "3750" },
  { word: "swore", f: "3750" },
  { word: "delay", f: "3749" },
  { word: "chase", f: "3747" },
  { word: "curse", f: "3745" },
  { word: "ripen", f: "3745" },
  { word: "tilde", f: "3745" },
  { word: "slope", f: "3742" },
  { word: "borne", f: "3742" },
  { word: "crave", f: "3741" },
  { word: "lithe", f: "3741" },
  { word: "carve", f: "3741" },
  { word: "gamer", f: "3738" },
  { word: "weary", f: "3731" },
  { word: "write", f: "3727" },
  { word: "retch", f: "3727" },
  { word: "taken", f: "3726" },
  { word: "space", f: "3725" },
  { word: "wrest", f: "3725" },
  { word: "camel", f: "3724" },
  { word: "death", f: "3723" },
  { word: "polar", f: "3718" },
  { word: "slept", f: "3717" },
  { word: "spelt", f: "3717" },
  { word: "ankle", f: "3716" },
  { word: "pause", f: "3715" },
  { word: "drake", f: "3714" },
  { word: "scone", f: "3708" },
  { word: "verso", f: "3708" },
  { word: "amber", f: "3708" },
  { word: "crept", f: "3705" },
  { word: "barge", f: "3703" },
  { word: "serif", f: "3702" },
  { word: "satyr", f: "3701" },
  { word: "adept", f: "3701" },
  { word: "stray", f: "3701" },
  { word: "smote", f: "3701" },
  { word: "artsy", f: "3701" },
  { word: "exalt", f: "3697" },
  { word: "radio", f: "3696" },
  { word: "salon", f: "3696" },
  { word: "erupt", f: "3695" },
  { word: "miner", f: "3694" },
  { word: "poise", f: "3694" },
  { word: "plead", f: "3691" },
  { word: "pedal", f: "3691" },
  { word: "heist", f: "3691" },
  { word: "reign", f: "3689" },
  { word: "cable", f: "3689" },
  { word: "arena", f: "3686" },
  { word: "rivet", f: "3685" },
  { word: "slide", f: "3685" },
  { word: "acorn", f: "3684" },
  { word: "scent", f: "3683" },
  { word: "matey", f: "3682" },
  { word: "skier", f: "3682" },
  { word: "meaty", f: "3682" },
  { word: "unite", f: "3675" },
  { word: "liver", f: "3675" },
  { word: "untie", f: "3675" },
  { word: "olden", f: "3674" },
  { word: "cider", f: "3673" },
  { word: "cried", f: "3673" },
  { word: "unset", f: "3673" },
  { word: "mealy", f: "3672" },
  { word: "triad", f: "3671" },
  { word: "slant", f: "3671" },
  { word: "spite", f: "3669" },
  { word: "horde", f: "3668" },
  { word: "moral", f: "3667" },
  { word: "wiser", f: "3667" },
  { word: "molar", f: "3667" },
  { word: "snake", f: "3666" },
  { word: "smelt", f: "3666" },
  { word: "sneak", f: "3666" },
  { word: "trash", f: "3665" },
  { word: "amuse", f: "3664" },
  { word: "rogue", f: "3664" },
  { word: "rouge", f: "3664" },
  { word: "shade", f: "3663" },
  { word: "elate", f: "3660" },
  { word: "latte", f: "3660" },
  { word: "spiel", f: "3659" },
  { word: "navel", f: "3659" },
  { word: "brine", f: "3659" },
  { word: "usage", f: "3659" },
  { word: "octal", f: "3658" },
  { word: "frame", f: "3657" },
  { word: "dance", f: "3657" },
  { word: "usher", f: "3657" },
  { word: "nosey", f: "3656" },
  { word: "owner", f: "3656" },
  { word: "facet", f: "3648" },
  { word: "price", f: "3647" },
  { word: "tapir", f: "3645" },
  { word: "reply", f: "3643" },
  { word: "strap", f: "3643" },
  { word: "abort", f: "3642" },
  { word: "spade", f: "3641" },
  { word: "medal", f: "3640" },
  { word: "adobe", f: "3640" },
  { word: "abode", f: "3640" },
  { word: "lunar", f: "3639" },
  { word: "fecal", f: "3638" },
  { word: "phase", f: "3637" },
  { word: "shape", f: "3637" },
  { word: "maker", f: "3637" },
  { word: "glade", f: "3635" },
  { word: "super", f: "3635" },
  { word: "purse", f: "3635" },
  { word: "lemur", f: "3634" },
  { word: "labor", f: "3632" },
  { word: "rayon", f: "3632" },
  { word: "pecan", f: "3631" },
  { word: "towel", f: "3630" },
  { word: "gruel", f: "3629" },
  { word: "abuse", f: "3629" },
  { word: "craze", f: "3628" },
  { word: "intro", f: "3628" },
  { word: "snort", f: "3626" },
  { word: "since", f: "3625" },
  { word: "saint", f: "3623" },
  { word: "satin", f: "3623" },
  { word: "stain", f: "3623" },
  { word: "shone", f: "3620" },
  { word: "smite", f: "3618" },
  { word: "azure", f: "3618" },
  { word: "wager", f: "3617" },
  { word: "otter", f: "3615" },
  { word: "retro", f: "3615" },
  { word: "ample", f: "3614" },
  { word: "maple", f: "3614" },
  { word: "slain", f: "3613" },
  { word: "snail", f: "3613" },
  { word: "naive", f: "3611" },
  { word: "bathe", f: "3611" },
  { word: "asset", f: "3610" },
  { word: "tease", f: "3610" },
  { word: "taste", f: "3610" },
  { word: "state", f: "3610" },
  { word: "brute", f: "3609" },
  { word: "tuber", f: "3609" },
  { word: "rebut", f: "3609" },
  { word: "finer", f: "3608" },
  { word: "smile", f: "3608" },
  { word: "slime", f: "3608" },
  { word: "ascot", f: "3608" },
  { word: "infer", f: "3608" },
  { word: "coast", f: "3608" },
  { word: "abled", f: "3605" },
  { word: "blade", f: "3605" },
  { word: "break", f: "3602" },
  { word: "brake", f: "3602" },
  { word: "baker", f: "3602" },
  { word: "hyena", f: "3601" },
  { word: "cairn", f: "3601" },
  { word: "lease", f: "3600" },
  { word: "easel", f: "3600" },
  { word: "opine", f: "3600" },
  { word: "adorn", f: "3600" },
  { word: "bluer", f: "3599" },
  { word: "melon", f: "3597" },
  { word: "lemon", f: "3597" },
  { word: "crime", f: "3596" },
  { word: "stoke", f: "3595" },
  { word: "force", f: "3593" },
  { word: "omega", f: "3593" },
  { word: "smart", f: "3592" },
  { word: "media", f: "3592" },
  { word: "homer", f: "3591" },
  { word: "racer", f: "3588" },
  { word: "shame", f: "3586" },
  { word: "leafy", f: "3586" },
  { word: "serum", f: "3584" },
  { word: "filet", f: "3582" },
  { word: "flora", f: "3581" },
  { word: "surge", f: "3579" },
  { word: "grail", f: "3579" },
  { word: "cutie", f: "3577" },
  { word: "inept", f: "3575" },
  { word: "grave", f: "3575" },
  { word: "apron", f: "3574" },
  { word: "spent", f: "3573" },
  { word: "loath", f: "3570" },
  { word: "under", f: "3567" },
  { word: "leaky", f: "3566" },
  { word: "grope", f: "3564" },
  { word: "pried", f: "3563" },
  { word: "pride", f: "3563" },
  { word: "noble", f: "3562" },
  { word: "cower", f: "3558" },
  { word: "gleam", f: "3558" },
  { word: "abide", f: "3557" },
  { word: "value", f: "3551" },
  { word: "freak", f: "3551" },
  { word: "rainy", f: "3549" },
  { word: "rebus", f: "3549" },
  { word: "brave", f: "3545" },
  { word: "snide", f: "3541" },
  { word: "foyer", f: "3541" },
  { word: "prune", f: "3541" },
  { word: "clerk", f: "3538" },
  { word: "stove", f: "3538" },
  { word: "shine", f: "3537" },
  { word: "wafer", f: "3536" },
  { word: "probe", f: "3534" },
  { word: "begat", f: "3533" },
  { word: "trite", f: "3532" },
  { word: "berth", f: "3531" },
  { word: "terse", f: "3530" },
  { word: "steer", f: "3530" },
  { word: "olive", f: "3530" },
  { word: "reset", f: "3530" },
  { word: "ester", f: "3530" },
  { word: "blame", f: "3528" },
  { word: "amble", f: "3528" },
  { word: "solve", f: "3528" },
  { word: "wheat", f: "3525" },
  { word: "nerdy", f: "3525" },
  { word: "bagel", f: "3523" },
  { word: "manor", f: "3523" },
  { word: "chose", f: "3522" },
  { word: "trawl", f: "3521" },
  { word: "salty", f: "3521" },
  { word: "often", f: "3521" },
  { word: "torus", f: "3518" },
  { word: "groan", f: "3518" },
  { word: "organ", f: "3518" },
  { word: "nadir", f: "3517" },
  { word: "drain", f: "3517" },
  { word: "cover", f: "3516" },
  { word: "eaten", f: "3516" },
  { word: "wreak", f: "3516" },
  { word: "spine", f: "3515" },
  { word: "whale", f: "3515" },
  { word: "snipe", f: "3515" },
  { word: "house", f: "3512" },
  { word: "felon", f: "3511" },
  { word: "shoal", f: "3510" },
  { word: "image", f: "3510" },
  { word: "comet", f: "3509" },
  { word: "dirge", f: "3507" },
  { word: "decay", f: "3507" },
  { word: "ridge", f: "3507" },
  { word: "flyer", f: "3506" },
  { word: "ounce", f: "3506" },
  { word: "dread", f: "3504" },
  { word: "valor", f: "3504" },
  { word: "edict", f: "3503" },
  { word: "suave", f: "3501" },
  { word: "token", f: "3501" },
  { word: "scope", f: "3500" },
  { word: "patio", f: "3500" },
  { word: "copse", f: "3500" },
  { word: "ethic", f: "3499" },
  { word: "flair", f: "3498" },
  { word: "roach", f: "3498" },
  { word: "frail", f: "3498" },
  { word: "chest", f: "3497" },
  { word: "amend", f: "3496" },
  { word: "grant", f: "3493" },
  { word: "gloat", f: "3492" },
  { word: "tidal", f: "3491" },
  { word: "baron", f: "3488" },
  { word: "prime", f: "3486" },
  { word: "stark", f: "3486" },
  { word: "shalt", f: "3485" },
  { word: "sperm", f: "3484" },
  { word: "ombre", f: "3483" },
  { word: "gripe", f: "3481" },
  { word: "shake", f: "3480" },
  { word: "parer", f: "3478" },
  { word: "paper", f: "3478" },
  { word: "bride", f: "3477" },
  { word: "flame", f: "3477" },
  { word: "story", f: "3476" },
  { word: "depot", f: "3476" },
  { word: "screw", f: "3473" },
  { word: "chart", f: "3473" },
  { word: "halve", f: "3473" },
  { word: "riser", f: "3472" },
  { word: "extol", f: "3472" },
  { word: "straw", f: "3471" },
  { word: "uncle", f: "3471" },
  { word: "crude", f: "3469" },
  { word: "knelt", f: "3466" },
  { word: "setup", f: "3465" },
  { word: "upset", f: "3465" },
  { word: "plait", f: "3465" },
  { word: "splat", f: "3463" },
  { word: "poker", f: "3463" },
  { word: "graze", f: "3462" },
  { word: "gazer", f: "3462" },
  { word: "bloat", f: "3462" },
  { word: "waver", f: "3459" },
  { word: "singe", f: "3459" },
  { word: "inane", f: "3458" },
  { word: "fiery", f: "3458" },
  { word: "speak", f: "3458" },
  { word: "pulse", f: "3455" },
  { word: "exact", f: "3455" },
  { word: "boule", f: "3454" },
  { word: "savor", f: "3454" },
  { word: "deity", f: "3451" },
  { word: "scary", f: "3449" },
  { word: "power", f: "3448" },
  { word: "poesy", f: "3448" },
  { word: "droit", f: "3446" },
  { word: "threw", f: "3445" },
  { word: "cheap", f: "3445" },
  { word: "peach", f: "3445" },
  { word: "fable", f: "3442" },
  { word: "yield", f: "3441" },
  { word: "staid", f: "3441" },
  { word: "short", f: "3440" },
  { word: "mouse", f: "3439" },
  { word: "enter", f: "3436" },
  { word: "grain", f: "3435" },
  { word: "novel", f: "3434" },
  { word: "drove", f: "3432" },
  { word: "zebra", f: "3432" },
  { word: "antic", f: "3431" },
  { word: "grime", f: "3430" },
  { word: "scant", f: "3429" },
  { word: "hover", f: "3428" },
  { word: "elfin", f: "3428" },
  { word: "forge", f: "3427" },
  { word: "equal", f: "3427" },
  { word: "decry", f: "3427" },
  { word: "rearm", f: "3427" },
  { word: "fried", f: "3426" },
  { word: "macro", f: "3425" },
  { word: "tardy", f: "3425" },
  { word: "piety", f: "3425" },
  { word: "cagey", f: "3425" },
  { word: "shave", f: "3423" },
  { word: "eager", f: "3422" },
  { word: "agree", f: "3422" },
  { word: "bleak", f: "3422" },
  { word: "viral", f: "3421" },
  { word: "rival", f: "3421" },
  { word: "cargo", f: "3420" },
  { word: "fresh", f: "3420" },
  { word: "acrid", f: "3419" },
  { word: "heady", f: "3419" },
  { word: "sport", f: "3418" },
  { word: "spice", f: "3417" },
  { word: "model", f: "3415" },
  { word: "chair", f: "3415" },
  { word: "mange", f: "3414" },
  { word: "hoard", f: "3414" },
  { word: "crash", f: "3413" },
  { word: "boast", f: "3412" },
  { word: "aloft", f: "3411" },
  { word: "float", f: "3411" },
  { word: "lodge", f: "3410" },
  { word: "guest", f: "3409" },
  { word: "liken", f: "3408" },
  { word: "prove", f: "3406" },
  { word: "brain", f: "3405" },
  { word: "twine", f: "3403" },
  { word: "guile", f: "3401" },
  { word: "above", f: "3400" },
  { word: "party", f: "3399" },
  { word: "tempo", f: "3399" },
  { word: "mower", f: "3397" },
  { word: "gavel", f: "3395" },
  { word: "tepid", f: "3393" },
  { word: "rebar", f: "3392" },
  { word: "scrap", f: "3391" },
  { word: "wider", f: "3391" },
  { word: "weird", f: "3391" },
  { word: "knead", f: "3390" },
  { word: "cobra", f: "3390" },
  { word: "cleft", f: "3388" },
  { word: "ovine", f: "3386" },
  { word: "shrew", f: "3385" },
  { word: "plied", f: "3383" },
  { word: "mural", f: "3380" },
  { word: "began", f: "3379" },
  { word: "inner", f: "3378" },
  { word: "flute", f: "3378" },
  { word: "broke", f: "3377" },
  { word: "nasty", f: "3377" },
  { word: "blast", f: "3377" },
  { word: "sneer", f: "3376" },
  { word: "honey", f: "3376" },
  { word: "loose", f: "3375" },
  { word: "scorn", f: "3374" },
  { word: "mayor", f: "3373" },
  { word: "flake", f: "3371" },
  { word: "awoke", f: "3371" },
  { word: "plant", f: "3369" },
  { word: "inlay", f: "3369" },
  { word: "diary", f: "3367" },
  { word: "dairy", f: "3367" },
  { word: "storm", f: "3367" },
  { word: "perch", f: "3365" },
  { word: "hairy", f: "3363" },
  { word: "corer", f: "3363" },
  { word: "aorta", f: "3361" },
  { word: "tarot", f: "3361" },
  { word: "hazel", f: "3360" },
  { word: "beach", f: "3359" },
  { word: "prude", f: "3359" },
  { word: "cease", f: "3358" },
  { word: "shirt", f: "3357" },
  { word: "alley", f: "3356" },
  { word: "mover", f: "3355" },
  { word: "shied", f: "3355" },
  { word: "ralph", f: "3353" },
  { word: "title", f: "3352" },
  { word: "elite", f: "3352" },
  { word: "guise", f: "3351" },
  { word: "steel", f: "3350" },
  { word: "mercy", f: "3350" },
  { word: "grove", f: "3350" },
  { word: "sleet", f: "3350" },
  { word: "diver", f: "3349" },
  { word: "drive", f: "3349" },
  { word: "tango", f: "3348" },
  { word: "tonga", f: "3348" },
  { word: "tweak", f: "3346" },
  { word: "piano", f: "3346" },
  { word: "covet", f: "3346" },
  { word: "north", f: "3346" },
  { word: "thorn", f: "3346" },
  { word: "stand", f: "3345" },
  { word: "niche", f: "3345" },
  { word: "grief", f: "3344" },
  { word: "swine", f: "3343" },
  { word: "sinew", f: "3343" },
  { word: "scram", f: "3340" },
  { word: "spray", f: "3339" },
  { word: "raspy", f: "3339" },
  { word: "exist", f: "3339" },
  { word: "erect", f: "3338" },
  { word: "along", f: "3338" },
  { word: "cigar", f: "3337" },
  { word: "clove", f: "3336" },
  { word: "lefty", f: "3336" },
  { word: "strip", f: "3335" },
  { word: "orbit", f: "3334" },
  { word: "eight", f: "3333" },
  { word: "spied", f: "3333" },
  { word: "golem", f: "3333" },
  { word: "heath", f: "3330" },
  { word: "theta", f: "3330" },
  { word: "shard", f: "3329" },
  { word: "haven", f: "3329" },
  { word: "utter", f: "3328" },
  { word: "fetus", f: "3328" },
  { word: "truer", f: "3328" },
  { word: "glide", f: "3327" },
  { word: "court", f: "3326" },
  { word: "altar", f: "3326" },
  { word: "sugar", f: "3325" },
  { word: "irony", f: "3324" },
  { word: "broil", f: "3324" },
  { word: "ladle", f: "3324" },
  { word: "viper", f: "3323" },
  { word: "paint", f: "3321" },
  { word: "unmet", f: "3320" },
  { word: "basil", f: "3319" },
  { word: "croak", f: "3319" },
  { word: "ranch", f: "3319" },
  { word: "ruler", f: "3318" },
  { word: "phone", f: "3318" },
  { word: "baton", f: "3318" },
  { word: "cyber", f: "3315" },
  { word: "brief", f: "3314" },
  { word: "fiber", f: "3314" },
  { word: "fibre", f: "3314" },
  { word: "craft", f: "3314" },
  { word: "hyper", f: "3313" },
  { word: "aloud", f: "3312" },
  { word: "beady", f: "3311" },
  { word: "plain", f: "3311" },
  { word: "lumen", f: "3310" },
  { word: "rapid", f: "3309" },
  { word: "chafe", f: "3308" },
  { word: "demur", f: "3308" },
  { word: "debit", f: "3307" },
  { word: "aware", f: "3306" },
  { word: "impel", f: "3306" },
  { word: "broad", f: "3306" },
  { word: "board", f: "3306" },
  { word: "stalk", f: "3306" },
  { word: "essay", f: "3306" },
  { word: "lunge", f: "3305" },
  { word: "buyer", f: "3305" },
  { word: "twice", f: "3305" },
  { word: "sharp", f: "3303" },
  { word: "money", f: "3303" },
  { word: "abhor", f: "3302" },
  { word: "stoic", f: "3300" },
  { word: "apple", f: "3298" },
  { word: "coupe", f: "3298" },
  { word: "lapel", f: "3298" },
  { word: "globe", f: "3298" },
  { word: "chute", f: "3295" },
  { word: "dowel", f: "3294" },
  { word: "track", f: "3294" },
  { word: "mason", f: "3293" },
  { word: "latch", f: "3293" },
  { word: "whole", f: "3290" },
  { word: "tramp", f: "3290" },
  { word: "clued", f: "3289" },
  { word: "voice", f: "3288" },
  { word: "rhino", f: "3288" },
  { word: "foray", f: "3287" },
  { word: "adult", f: "3287" },
  { word: "retry", f: "3286" },
  { word: "shorn", f: "3286" },
  { word: "askew", f: "3286" },
  { word: "decoy", f: "3282" },
  { word: "glaze", f: "3282" },
  { word: "frost", f: "3281" },
  { word: "today", f: "3280" },
  { word: "crier", f: "3280" },
  { word: "odder", f: "3279" },
  { word: "rodeo", f: "3279" },
  { word: "order", f: "3279" },
  { word: "erode", f: "3279" },
  { word: "tiara", f: "3278" },
  { word: "trait", f: "3278" },
  { word: "cress", f: "3278" },
  { word: "scree", f: "3278" },
  { word: "purge", f: "3277" },
  { word: "leery", f: "3276" },
  { word: "start", f: "3276" },
  { word: "voila", f: "3276" },
  { word: "viola", f: "3276" },
  { word: "salvo", f: "3274" },
  { word: "mince", f: "3272" },
  { word: "randy", f: "3271" },
  { word: "demon", f: "3271" },
  { word: "piney", f: "3271" },
  { word: "crawl", f: "3269" },
  { word: "scaly", f: "3269" },
  { word: "dwelt", f: "3269" },
  { word: "curio", f: "3268" },
  { word: "reuse", f: "3268" },
  { word: "chaos", f: "3268" },
  { word: "surer", f: "3268" },
  { word: "ebony", f: "3268" },
  { word: "boney", f: "3268" },
  { word: "giver", f: "3267" },
  { word: "scour", f: "3266" },
  { word: "giant", f: "3265" },
  { word: "audio", f: "3264" },
  { word: "angst", f: "3263" },
  { word: "evict", f: "3263" },
  { word: "rhyme", f: "3262" },
  { word: "stork", f: "3261" },
  { word: "sloth", f: "3260" },
  { word: "fetid", f: "3256" },
  { word: "biome", f: "3255" },
  { word: "align", f: "3255" },
  { word: "deter", f: "3254" },
  { word: "scarf", f: "3254" },
  { word: "slang", f: "3253" },
  { word: "marsh", f: "3252" },
  { word: "agate", f: "3252" },
  { word: "thief", f: "3252" },
  { word: "blaze", f: "3252" },
  { word: "vital", f: "3251" },
  { word: "vegan", f: "3251" },
  { word: "three", f: "3250" },
  { word: "there", f: "3250" },
  { word: "ether", f: "3250" },
  { word: "flirt", f: "3248" },
  { word: "torch", f: "3248" },
  { word: "hovel", f: "3248" },
  { word: "worst", f: "3246" },
  { word: "field", f: "3246" },
  { word: "elder", f: "3244" },
  { word: "waist", f: "3243" },
  { word: "algae", f: "3242" },
  { word: "eagle", f: "3242" },
  { word: "legal", f: "3242" },
  { word: "crust", f: "3241" },
  { word: "print", f: "3241" },
  { word: "flesh", f: "3240" },
  { word: "shelf", f: "3240" },
  { word: "pilot", f: "3240" },
  { word: "whose", f: "3240" },
  { word: "maize", f: "3239" },
  { word: "audit", f: "3239" },
  { word: "truly", f: "3239" },
  { word: "spend", f: "3237" },
  { word: "scald", f: "3237" },
  { word: "maybe", f: "3234" },
  { word: "spoke", f: "3233" },
  { word: "clash", f: "3233" },
  { word: "noose", f: "3231" },
  { word: "derby", f: "3231" },
  { word: "waive", f: "3231" },
  { word: "draft", f: "3230" },
  { word: "curve", f: "3229" },
  { word: "warty", f: "3227" },
  { word: "grasp", f: "3225" },
  { word: "rabid", f: "3223" },
  { word: "braid", f: "3223" },
  { word: "abate", f: "3222" },
  { word: "adopt", f: "3222" },
  { word: "epoch", f: "3220" },
  { word: "aptly", f: "3219" },
  { word: "repel", f: "3218" },
  { word: "leper", f: "3218" },
  { word: "brash", f: "3217" },
  { word: "white", f: "3217" },
  { word: "eying", f: "3215" },
  { word: "bilge", f: "3215" },
  { word: "minor", f: "3215" },
  { word: "twirl", f: "3213" },
  { word: "hoist", f: "3212" },
  { word: "quote", f: "3212" },
  { word: "label", f: "3212" },
  { word: "scalp", f: "3211" },
  { word: "clasp", f: "3211" },
  { word: "wield", f: "3211" },
  { word: "about", f: "3210" },
  { word: "ovary", f: "3210" },
  { word: "groin", f: "3210" },
  { word: "prize", f: "3210" },
  { word: "while", f: "3207" },
  { word: "tonic", f: "3206" },
  { word: "solid", f: "3206" },
  { word: "tense", f: "3206" },
  { word: "welsh", f: "3205" },
  { word: "fairy", f: "3204" },
  { word: "boxer", f: "3204" },
  { word: "video", f: "3204" },
  { word: "vista", f: "3201" },
  { word: "urban", f: "3201" },
  { word: "blend", f: "3201" },
  { word: "dicey", f: "3199" },
  { word: "shove", f: "3198" },
  { word: "linen", f: "3198" },
  { word: "first", f: "3198" },
  { word: "badge", f: "3197" },
  { word: "gorge", f: "3197" },
  { word: "bloke", f: "3197" },
  { word: "masse", f: "3197" },
  { word: "roger", f: "3197" },
  { word: "rider", f: "3196" },
  { word: "drier", f: "3196" },
  { word: "dried", f: "3196" },
  { word: "dress", f: "3194" },
  { word: "snout", f: "3194" },
  { word: "soapy", f: "3194" },
  { word: "loamy", f: "3193" },
  { word: "octet", f: "3193" },
  { word: "swept", f: "3193" },
  { word: "coven", f: "3192" },
  { word: "lyric", f: "3191" },
  { word: "hasty", f: "3191" },
  { word: "wrath", f: "3191" },
  { word: "choir", f: "3190" },
  { word: "posit", f: "3190" },
  { word: "sheer", f: "3190" },
  { word: "rusty", f: "3189" },
  { word: "angry", f: "3189" },
  { word: "gnome", f: "3189" },
  { word: "denim", f: "3188" },
  { word: "daily", f: "3187" },
  { word: "front", f: "3187" },
  { word: "drawl", f: "3185" },
  { word: "exult", f: "3185" },
  { word: "sadly", f: "3185" },
  { word: "faint", f: "3184" },
  { word: "deign", f: "3183" },
  { word: "cello", f: "3183" },
  { word: "bowel", f: "3182" },
  { word: "smoke", f: "3182" },
  { word: "below", f: "3182" },
  { word: "elbow", f: "3182" },
  { word: "atoll", f: "3181" },
  { word: "total", f: "3181" },
  { word: "allot", f: "3181" },
  { word: "robin", f: "3180" },
  { word: "spoil", f: "3180" },
  { word: "heavy", f: "3179" },
  { word: "vicar", f: "3179" },
  { word: "neigh", f: "3179" },
  { word: "surly", f: "3179" },
  { word: "hinge", f: "3179" },
  { word: "skirt", f: "3178" },
  { word: "meter", f: "3177" },
  { word: "aunty", f: "3175" },
  { word: "basin", f: "3175" },
  { word: "tubal", f: "3175" },
  { word: "final", f: "3174" },
  { word: "rerun", f: "3174" },
  { word: "greet", f: "3172" },
  { word: "sheik", f: "3172" },
  { word: "egret", f: "3172" },
  { word: "dopey", f: "3172" },
  { word: "riper", f: "3170" },
  { word: "piper", f: "3170" },
  { word: "glove", f: "3170" },
  { word: "patsy", f: "3169" },
  { word: "devil", f: "3169" },
  { word: "pasty", f: "3169" },
  { word: "press", f: "3168" },
  { word: "spree", f: "3168" },
  { word: "tumor", f: "3165" },
  { word: "wrist", f: "3163" },
  { word: "chide", f: "3163" },
  { word: "abase", f: "3162" },
  { word: "claim", f: "3162" },
  { word: "stank", f: "3162" },
  { word: "fella", f: "3161" },
  { word: "until", f: "3161" },
  { word: "modal", f: "3161" },
  { word: "unlit", f: "3161" },
  { word: "grout", f: "3160" },
  { word: "palsy", f: "3159" },
  { word: "focal", f: "3159" },
  { word: "elect", f: "3158" },
  { word: "grand", f: "3157" },
  { word: "froze", f: "3156" },
  { word: "split", f: "3155" },
  { word: "spilt", f: "3155" },
  { word: "apnea", f: "3154" },
  { word: "swirl", f: "3153" },
  { word: "vapor", f: "3152" },
  { word: "stony", f: "3152" },
  { word: "wince", f: "3151" },
  { word: "endow", f: "3150" },
  { word: "knave", f: "3150" },
  { word: "spike", f: "3150" },
  { word: "chant", f: "3149" },
  { word: "afoul", f: "3149" },
  { word: "lurid", f: "3149" },
  { word: "graft", f: "3148" },
  { word: "mauve", f: "3148" },
  { word: "tulle", f: "3148" },
  { word: "newly", f: "3147" },
  { word: "visor", f: "3146" },
  { word: "sonic", f: "3146" },
  { word: "scion", f: "3146" },
  { word: "clout", f: "3146" },
  { word: "detox", f: "3146" },
  { word: "shark", f: "3146" },
  { word: "femur", f: "3145" },
  { word: "dunce", f: "3145" },
  { word: "befit", f: "3144" },
  { word: "daunt", f: "3143" },
  { word: "vague", f: "3143" },
  { word: "beret", f: "3142" },
  { word: "crank", f: "3140" },
  { word: "envoy", f: "3140" },
  { word: "cloak", f: "3139" },
  { word: "moist", f: "3139" },
  { word: "haunt", f: "3139" },
  { word: "daisy", f: "3137" },
  { word: "chard", f: "3137" },
  { word: "perky", f: "3134" },
  { word: "rebel", f: "3132" },
  { word: "toast", f: "3131" },
  { word: "spurt", f: "3131" },
  { word: "turbo", f: "3130" },
  { word: "crony", f: "3130" },
  { word: "corny", f: "3130" },
  { word: "quiet", f: "3129" },
  { word: "plaid", f: "3129" },
  { word: "quite", f: "3129" },
  { word: "brand", f: "3127" },
  { word: "quest", f: "3127" },
  { word: "movie", f: "3127" },
  { word: "fault", f: "3124" },
  { word: "spark", f: "3124" },
  { word: "joker", f: "3123" },
  { word: "slurp", f: "3121" },
  { word: "lasso", f: "3121" },
  { word: "amity", f: "3120" },
  { word: "micro", f: "3117" },
  { word: "dirty", f: "3117" },
  { word: "offer", f: "3116" },
  { word: "depth", f: "3111" },
  { word: "rarer", f: "3111" },
  { word: "glory", f: "3108" },
  { word: "gaily", f: "3105" },
  { word: "debut", f: "3103" },
  { word: "enema", f: "3103" },
  { word: "quart", f: "3103" },
  { word: "fiend", f: "3102" },
  { word: "plume", f: "3102" },
  { word: "troll", f: "3101" },
  { word: "belch", f: "3099" },
  { word: "anvil", f: "3097" },
  { word: "zesty", f: "3096" },
  { word: "stall", f: "3096" },
  { word: "point", f: "3096" },
  { word: "scout", f: "3096" },
  { word: "pinto", f: "3096" },
  { word: "hello", f: "3095" },
  { word: "blurt", f: "3095" },
  { word: "noisy", f: "3094" },
  { word: "thyme", f: "3092" },
  { word: "sworn", f: "3092" },
  { word: "china", f: "3091" },
  { word: "chain", f: "3091" },
  { word: "medic", f: "3090" },
  { word: "incur", f: "3089" },
  { word: "round", f: "3088" },
  { word: "admit", f: "3088" },
  { word: "oxide", f: "3088" },
  { word: "chime", f: "3086" },
  { word: "locus", f: "3086" },
  { word: "hardy", f: "3085" },
  { word: "carat", f: "3084" },
  { word: "tract", f: "3084" },
  { word: "leave", f: "3084" },
  { word: "valve", f: "3084" },
  { word: "bribe", f: "3084" },
  { word: "crisp", f: "3083" },
  { word: "vocal", f: "3082" },
  { word: "rower", f: "3081" },
  { word: "wooer", f: "3081" },
  { word: "third", f: "3081" },
  { word: "cache", f: "3078" },
  { word: "basic", f: "3077" },
  { word: "recur", f: "3076" },
  { word: "guide", f: "3075" },
  { word: "preen", f: "3074" },
  { word: "brawl", f: "3073" },
  { word: "women", f: "3073" },
  { word: "elope", f: "3073" },
  { word: "binge", f: "3071" },
  { word: "being", f: "3071" },
  { word: "begin", f: "3071" },
  { word: "fixer", f: "3070" },
  { word: "empty", f: "3070" },
  { word: "flour", f: "3069" },
  { word: "panic", f: "3069" },
  { word: "cloth", f: "3068" },
  { word: "widen", f: "3067" },
  { word: "zonal", f: "3067" },
  { word: "bacon", f: "3066" },
  { word: "bravo", f: "3066" },
  { word: "stack", f: "3064" },
  { word: "rural", f: "3064" },
  { word: "choke", f: "3063" },
  { word: "whine", f: "3063" },
  { word: "clang", f: "3061" },
  { word: "gaunt", f: "3061" },
  { word: "march", f: "3060" },
  { word: "stamp", f: "3060" },
  { word: "charm", f: "3060" },
  { word: "harpy", f: "3059" },
  { word: "swarm", f: "3058" },
  { word: "fetch", f: "3058" },
  { word: "testy", f: "3056" },
  { word: "peace", f: "3056" },
  { word: "drank", f: "3056" },
  { word: "slack", f: "3054" },
  { word: "vowel", f: "3054" },
  { word: "query", f: "3053" },
  { word: "foist", f: "3053" },
  { word: "broth", f: "3052" },
  { word: "throb", f: "3052" },
  { word: "diode", f: "3051" },
  { word: "torso", f: "3051" },
  { word: "roost", f: "3051" },
  { word: "psalm", f: "3050" },
  { word: "helix", f: "3049" },
  { word: "guard", f: "3049" },
  { word: "habit", f: "3049" },
  { word: "vault", f: "3047" },
  { word: "burst", f: "3045" },
  { word: "agony", f: "3044" },
  { word: "bused", f: "3043" },
  { word: "covey", f: "3042" },
  { word: "horny", f: "3042" },
  { word: "drawn", f: "3041" },
  { word: "sandy", f: "3041" },
  { word: "ingot", f: "3040" },
  { word: "issue", f: "3040" },
  { word: "rover", f: "3039" },
  { word: "cramp", f: "3038" },
  { word: "forty", f: "3037" },
  { word: "lousy", f: "3034" },
  { word: "ratty", f: "3032" },
  { word: "totem", f: "3032" },
  { word: "venom", f: "3031" },
  { word: "lingo", f: "3030" },
  { word: "gauze", f: "3030" },
  { word: "login", f: "3030" },
  { word: "prank", f: "3030" },
  { word: "bicep", f: "3029" },
  { word: "polka", f: "3029" },
  { word: "pixel", f: "3027" },
  { word: "circa", f: "3026" },
  { word: "girly", f: "3025" },
  { word: "ardor", f: "3025" },
  { word: "crass", f: "3024" },
  { word: "steed", f: "3024" },
  { word: "posse", f: "3023" },
  { word: "rally", f: "3022" },
  { word: "tithe", f: "3022" },
  { word: "these", f: "3020" },
  { word: "sheet", f: "3020" },
  { word: "feign", f: "3020" },
  { word: "tangy", f: "3019" },
  { word: "caput", f: "3019" },
  { word: "waxen", f: "3019" },
  { word: "manic", f: "3018" },
  { word: "green", f: "3018" },
  { word: "genre", f: "3018" },
  { word: "saucy", f: "3017" },
  { word: "nomad", f: "3017" },
  { word: "elide", f: "3016" },
  { word: "hotly", f: "3016" },
  { word: "axion", f: "3016" },
  { word: "favor", f: "3015" },
  { word: "pansy", f: "3015" },
  { word: "prawn", f: "3015" },
  { word: "enjoy", f: "3014" },
  { word: "manly", f: "3014" },
  { word: "wreck", f: "3014" },
  { word: "welch", f: "3013" },
  { word: "gonad", f: "3012" },
  { word: "scold", f: "3012" },
  { word: "mourn", f: "3011" },
  { word: "bugle", f: "3011" },
  { word: "bulge", f: "3011" },
  { word: "shell", f: "3010" },
  { word: "lusty", f: "3009" },
  { word: "south", f: "3008" },
  { word: "shout", f: "3008" },
  { word: "primo", f: "3007" },
  { word: "hefty", f: "3006" },
  { word: "glint", f: "3005" },
  { word: "mecca", f: "3005" },
  { word: "mirth", f: "3004" },
  { word: "revel", f: "3004" },
  { word: "lever", f: "3004" },
  { word: "payee", f: "3004" },
  { word: "count", f: "3002" },
  { word: "creed", f: "3002" },
  { word: "natal", f: "3002" },
  { word: "forth", f: "3001" },
  { word: "froth", f: "3001" },
  { word: "chief", f: "3000" },
  { word: "eking", f: "3000" },
  { word: "yacht", f: "2999" },
  { word: "right", f: "2999" },
  { word: "cavil", f: "2999" },
  { word: "girth", f: "2999" },
  { word: "optic", f: "2998" },
  { word: "topic", f: "2998" },
  { word: "cheer", f: "2998" },
  { word: "faith", f: "2998" },
  { word: "steep", f: "2998" },
  { word: "sewer", f: "2996" },
  { word: "shaft", f: "2996" },
  { word: "fruit", f: "2996" },
  { word: "dozen", f: "2995" },
  { word: "ahead", f: "2994" },
  { word: "ruder", f: "2992" },
  { word: "udder", f: "2992" },
  { word: "gauge", f: "2990" },
  { word: "sleep", f: "2988" },
  { word: "spell", f: "2988" },
  { word: "curly", f: "2987" },
  { word: "spout", f: "2986" },
  { word: "flash", f: "2986" },
  { word: "trick", f: "2986" },
  { word: "gecko", f: "2985" },
  { word: "moult", f: "2985" },
  { word: "cabin", f: "2983" },
  { word: "grunt", f: "2981" },
  { word: "savoy", f: "2980" },
  { word: "nudge", f: "2979" },
  { word: "spurn", f: "2977" },
  { word: "gland", f: "2977" },
  { word: "creep", f: "2976" },
  { word: "crepe", f: "2976" },
  { word: "duvet", f: "2975" },
  { word: "major", f: "2975" },
  { word: "apart", f: "2974" },
  { word: "moose", f: "2972" },
  { word: "birth", f: "2969" },
  { word: "fraud", f: "2968" },
  { word: "imbue", f: "2968" },
  { word: "goose", f: "2967" },
  { word: "woken", f: "2967" },
  { word: "throw", f: "2966" },
  { word: "worth", f: "2966" },
  { word: "puree", f: "2966" },
  { word: "upper", f: "2966" },
  { word: "purer", f: "2966" },
  { word: "rupee", f: "2966" },
  { word: "flume", f: "2965" },
  { word: "croup", f: "2964" },
  { word: "disco", f: "2964" },
  { word: "betel", f: "2962" },
  { word: "swath", f: "2961" },
  { word: "world", f: "2960" },
  { word: "clank", f: "2960" },
  { word: "aglow", f: "2958" },
  { word: "quota", f: "2958" },
  { word: "speck", f: "2956" },
  { word: "river", f: "2956" },
  { word: "niece", f: "2956" },
  { word: "bench", f: "2955" },
  { word: "sting", f: "2955" },
  { word: "serve", f: "2954" },
  { word: "verse", f: "2954" },
  { word: "taint", f: "2954" },
  { word: "titan", f: "2954" },
  { word: "scene", f: "2954" },
  { word: "sever", f: "2954" },
  { word: "tulip", f: "2953" },
  { word: "edify", f: "2952" },
  { word: "lurch", f: "2951" },
  { word: "shawl", f: "2951" },
  { word: "brunt", f: "2951" },
  { word: "burnt", f: "2951" },
  { word: "bison", f: "2950" },
  { word: "avoid", f: "2950" },
  { word: "borax", f: "2950" },
  { word: "reedy", f: "2950" },
  { word: "dryer", f: "2950" },
  { word: "armor", f: "2948" },
  { word: "aroma", f: "2948" },
  { word: "bland", f: "2947" },
  { word: "sigma", f: "2946" },
  { word: "ennui", f: "2946" },
  { word: "sling", f: "2945" },
  { word: "graph", f: "2945" },
  { word: "ensue", f: "2944" },
  { word: "agora", f: "2943" },
  { word: "given", f: "2943" },
  { word: "radii", f: "2942" },
  { word: "rumba", f: "2942" },
  { word: "nasal", f: "2942" },
  { word: "umbra", f: "2942" },
  { word: "patch", f: "2941" },
  { word: "hymen", f: "2938" },
  { word: "smell", f: "2937" },
  { word: "obese", f: "2937" },
  { word: "harsh", f: "2936" },
  { word: "mango", f: "2935" },
  { word: "among", f: "2935" },
  { word: "admin", f: "2934" },
  { word: "prowl", f: "2934" },
  { word: "liege", f: "2934" },
  { word: "logic", f: "2932" },
  { word: "gusto", f: "2930" },
  { word: "brawn", f: "2929" },
  { word: "local", f: "2929" },
  { word: "tenth", f: "2926" },
  { word: "creme", f: "2925" },
  { word: "notch", f: "2924" },
  { word: "flint", f: "2924" },
  { word: "midge", f: "2924" },
  { word: "gnash", f: "2923" },
  { word: "prism", f: "2922" },
  { word: "drift", f: "2922" },
  { word: "knife", f: "2919" },
  { word: "tunic", f: "2919" },
  { word: "abbey", f: "2918" },
  { word: "vogue", f: "2918" },
  { word: "quake", f: "2918" },
  { word: "donut", f: "2918" },
  { word: "sprig", f: "2917" },
  { word: "adage", f: "2916" },
  { word: "valid", f: "2915" },
  { word: "macho", f: "2915" },
  { word: "mocha", f: "2915" },
  { word: "arbor", f: "2913" },
  { word: "alarm", f: "2913" },
  { word: "chord", f: "2912" },
  { word: "beset", f: "2912" },
  { word: "fleet", f: "2911" },
  { word: "woven", f: "2910" },
  { word: "snack", f: "2910" },
  { word: "sword", f: "2910" },
  { word: "index", f: "2909" },
  { word: "lanky", f: "2908" },
  { word: "sumac", f: "2908" },
  { word: "prong", f: "2906" },
  { word: "bayou", f: "2906" },
  { word: "bible", f: "2904" },
  { word: "pesky", f: "2904" },
  { word: "belie", f: "2904" },
  { word: "libel", f: "2904" },
  { word: "aping", f: "2903" },
  { word: "vaunt", f: "2903" },
  { word: "tawny", f: "2903" },
  { word: "whelp", f: "2903" },
  { word: "ivory", f: "2902" },
  { word: "renew", f: "2902" },
  { word: "newer", f: "2902" },
  { word: "bless", f: "2902" },
  { word: "crush", f: "2901" },
  { word: "crown", f: "2900" },
  { word: "flout", f: "2899" },
  { word: "unfed", f: "2898" },
  { word: "exert", f: "2898" },
  { word: "crypt", f: "2897" },
  { word: "guilt", f: "2897" },
  { word: "frank", f: "2893" },
  { word: "agape", f: "2890" },
  { word: "match", f: "2890" },
  { word: "porch", f: "2886" },
  { word: "error", f: "2886" },
  { word: "datum", f: "2884" },
  { word: "siege", f: "2884" },
  { word: "thank", f: "2882" },
  { word: "proud", f: "2880" },
  { word: "trunk", f: "2880" },
  { word: "growl", f: "2878" },
  { word: "alloy", f: "2877" },
  { word: "loyal", f: "2877" },
  { word: "juice", f: "2875" },
  { word: "risky", f: "2874" },
  { word: "merry", f: "2873" },
  { word: "whirl", f: "2873" },
  { word: "squat", f: "2873" },
  { word: "scuba", f: "2873" },
  { word: "stool", f: "2871" },
  { word: "dense", f: "2870" },
  { word: "fleck", f: "2869" },
  { word: "wench", f: "2869" },
  { word: "topaz", f: "2869" },
  { word: "unity", f: "2867" },
  { word: "built", f: "2867" },
  { word: "begun", f: "2867" },
  { word: "sheen", f: "2866" },
  { word: "laugh", f: "2865" },
  { word: "quail", f: "2865" },
  { word: "unwed", f: "2863" },
  { word: "tipsy", f: "2861" },
  { word: "never", f: "2860" },
  { word: "nerve", f: "2860" },
  { word: "hydro", f: "2860" },
  { word: "fluke", f: "2859" },
  { word: "virus", f: "2859" },
  { word: "sound", f: "2858" },
  { word: "snaky", f: "2858" },
  { word: "grass", f: "2858" },
  { word: "clamp", f: "2858" },
  { word: "lofty", f: "2857" },
  { word: "attic", f: "2856" },
  { word: "tacit", f: "2856" },
  { word: "cacti", f: "2856" },
  { word: "batch", f: "2855" },
  { word: "shady", f: "2855" },
  { word: "cycle", f: "2854" },
  { word: "stink", f: "2854" },
  { word: "ghost", f: "2852" },
  { word: "tally", f: "2852" },
  { word: "caulk", f: "2852" },
  { word: "briny", f: "2851" },
  { word: "frond", f: "2851" },
  { word: "plank", f: "2850" },
  { word: "joist", f: "2850" },
  { word: "grind", f: "2849" },
  { word: "trout", f: "2849" },
  { word: "candy", f: "2849" },
  { word: "tutor", f: "2849" },
  { word: "color", f: "2849" },
  { word: "iliac", f: "2846" },
  { word: "lilac", f: "2846" },
  { word: "class", f: "2844" },
  { word: "slink", f: "2844" },
  { word: "mount", f: "2841" },
  { word: "stock", f: "2839" },
  { word: "berry", f: "2838" },
  { word: "shirk", f: "2838" },
  { word: "greed", f: "2836" },
  { word: "stomp", f: "2835" },
  { word: "usual", f: "2834" },
  { word: "madly", f: "2832" },
  { word: "sleek", f: "2831" },
  { word: "rabbi", f: "2830" },
  { word: "briar", f: "2830" },
  { word: "chasm", f: "2830" },
  { word: "swami", f: "2830" },
  { word: "shunt", f: "2829" },
  { word: "brass", f: "2828" },
  { word: "scrum", f: "2828" },
  { word: "syrup", f: "2827" },
  { word: "arrow", f: "2827" },
  { word: "sweet", f: "2826" },
  { word: "humor", f: "2825" },
  { word: "annex", f: "2824" },
  { word: "gourd", f: "2824" },
  { word: "etude", f: "2822" },
  { word: "shank", f: "2822" },
  { word: "rough", f: "2820" },
  { word: "crazy", f: "2820" },
  { word: "tacky", f: "2820" },
  { word: "creek", f: "2819" },
  { word: "woman", f: "2819" },
  { word: "light", f: "2819" },
  { word: "leech", f: "2818" },
  { word: "swell", f: "2816" },
  { word: "drown", f: "2816" },
  { word: "synod", f: "2816" },
  { word: "touch", f: "2816" },
  { word: "epoxy", f: "2816" },
  { word: "quasi", f: "2815" },
  { word: "scowl", f: "2814" },
  { word: "wagon", f: "2814" },
  { word: "elude", f: "2812" },
  { word: "cloud", f: "2810" },
  { word: "could", f: "2810" },
  { word: "input", f: "2809" },
  { word: "scamp", f: "2808" },
  { word: "flask", f: "2807" },
  { word: "churn", f: "2807" },
  { word: "amply", f: "2806" },
  { word: "breed", f: "2806" },
  { word: "chirp", f: "2803" },
  { word: "eerie", f: "2803" },
  { word: "gamut", f: "2802" },
  { word: "tasty", f: "2802" },
  { word: "spank", f: "2800" },
  { word: "thrum", f: "2800" },
  { word: "slimy", f: "2800" },
  { word: "aphid", f: "2799" },
  { word: "weigh", f: "2799" },
  { word: "cross", f: "2799" },
  { word: "group", f: "2798" },
  { word: "badly", f: "2797" },
  { word: "lorry", f: "2797" },
  { word: "jerky", f: "2794" },
  { word: "semen", f: "2793" },
  { word: "scrub", f: "2793" },
  { word: "sally", f: "2792" },
  { word: "burly", f: "2791" },
  { word: "genie", f: "2790" },
  { word: "twang", f: "2789" },
  { word: "still", f: "2788" },
  { word: "vigor", f: "2788" },
  { word: "stilt", f: "2788" },
  { word: "ferry", f: "2787" },
  { word: "canon", f: "2785" },
  { word: "spawn", f: "2785" },
  { word: "truck", f: "2782" },
  { word: "carry", f: "2780" },
  { word: "teddy", f: "2780" },
  { word: "friar", f: "2779" },
  { word: "midst", f: "2778" },
  { word: "trump", f: "2778" },
  { word: "junta", f: "2777" },
  { word: "jaunt", f: "2777" },
  { word: "chalk", f: "2774" },
  { word: "smith", f: "2774" },
  { word: "blunt", f: "2771" },
  { word: "sight", f: "2769" },
  { word: "watch", f: "2769" },
  { word: "chess", f: "2768" },
  { word: "pique", f: "2767" },
  { word: "gravy", f: "2767" },
  { word: "equip", f: "2767" },
  { word: "toxin", f: "2766" },
  { word: "stash", f: "2766" },
  { word: "smirk", f: "2765" },
  { word: "rocky", f: "2765" },
  { word: "drool", f: "2765" },
  { word: "droll", f: "2765" },
  { word: "gouge", f: "2765" },
  { word: "blank", f: "2764" },
  { word: "strut", f: "2764" },
  { word: "trust", f: "2764" },
  { word: "youth", f: "2764" },
  { word: "truss", f: "2764" },
  { word: "month", f: "2763" },
  { word: "album", f: "2762" },
  { word: "handy", f: "2761" },
  { word: "wrack", f: "2760" },
  { word: "salad", f: "2760" },
  { word: "merge", f: "2759" },
  { word: "evade", f: "2758" },
  { word: "thong", f: "2758" },
  { word: "axiom", f: "2757" },
  { word: "slash", f: "2756" },
  { word: "shall", f: "2756" },
  { word: "stick", f: "2756" },
  { word: "joint", f: "2756" },
  { word: "freed", f: "2755" },
  { word: "defer", f: "2755" },
  { word: "nobly", f: "2754" },
  { word: "heave", f: "2754" },
  { word: "magic", f: "2754" },
  { word: "petty", f: "2754" },
  { word: "gaffe", f: "2753" },
  { word: "dogma", f: "2753" },
  { word: "cling", f: "2753" },
  { word: "havoc", f: "2752" },
  { word: "revue", f: "2752" },
  { word: "stung", f: "2751" },
  { word: "taunt", f: "2750" },
  { word: "larva", f: "2750" },
  { word: "canal", f: "2750" },
  { word: "troop", f: "2749" },
  { word: "drink", f: "2748" },
  { word: "piece", f: "2748" },
  { word: "sorry", f: "2747" },
  { word: "bigot", f: "2746" },
  { word: "slick", f: "2746" },
  { word: "bonus", f: "2746" },
  { word: "pasta", f: "2744" },
  { word: "abbot", f: "2743" },
  { word: "taboo", f: "2743" },
  { word: "pouty", f: "2742" },
  { word: "limbo", f: "2741" },
  { word: "slung", f: "2741" },
  { word: "annul", f: "2740" },
  { word: "filth", f: "2738" },
  { word: "kneel", f: "2737" },
  { word: "bring", f: "2737" },
  { word: "shrug", f: "2735" },
  { word: "grown", f: "2734" },
  { word: "wrong", f: "2734" },
  { word: "annoy", f: "2733" },
  { word: "crimp", f: "2730" },
  { word: "brisk", f: "2730" },
  { word: "shiny", f: "2729" },
  { word: "ember", f: "2729" },
  { word: "lucid", f: "2727" },
  { word: "human", f: "2726" },
  { word: "sieve", f: "2726" },
  { word: "morph", f: "2725" },
  { word: "shack", f: "2724" },
  { word: "gooey", f: "2723" },
  { word: "blond", f: "2722" },
  { word: "tryst", f: "2722" },
  { word: "clown", f: "2720" },
  { word: "seedy", f: "2720" },
  { word: "balmy", f: "2720" },
  { word: "crowd", f: "2718" },
  { word: "birch", f: "2717" },
  { word: "minty", f: "2716" },
  { word: "where", f: "2716" },
  { word: "dross", f: "2715" },
  { word: "flank", f: "2713" },
  { word: "tying", f: "2711" },
  { word: "every", f: "2710" },
  { word: "butte", f: "2710" },
  { word: "spiny", f: "2707" },
  { word: "brush", f: "2705" },
  { word: "shrub", f: "2705" },
  { word: "foamy", f: "2704" },
  { word: "brown", f: "2704" },
  { word: "doing", f: "2704" },
  { word: "dingo", f: "2704" },
  { word: "kebab", f: "2703" },
  { word: "lying", f: "2701" },
  { word: "motif", f: "2700" },
  { word: "minus", f: "2698" },
  { word: "motor", f: "2698" },
  { word: "modem", f: "2696" },
  { word: "dwarf", f: "2696" },
  { word: "silky", f: "2694" },
  { word: "suing", f: "2693" },
  { word: "using", f: "2693" },
  { word: "afoot", f: "2692" },
  { word: "harry", f: "2692" },
  { word: "wharf", f: "2692" },
  { word: "itchy", f: "2691" },
  { word: "dodge", f: "2691" },
  { word: "prior", f: "2691" },
  { word: "sauna", f: "2690" },
  { word: "event", f: "2690" },
  { word: "shift", f: "2688" },
  { word: "elegy", f: "2688" },
  { word: "leggy", f: "2688" },
  { word: "fancy", f: "2686" },
  { word: "debug", f: "2685" },
  { word: "budge", f: "2685" },
  { word: "study", f: "2683" },
  { word: "dusty", f: "2683" },
  { word: "small", f: "2683" },
  { word: "drill", f: "2682" },
  { word: "offal", f: "2682" },
  { word: "aloof", f: "2682" },
  { word: "guess", f: "2680" },
  { word: "segue", f: "2680" },
  { word: "frisk", f: "2679" },
  { word: "glass", f: "2678" },
  { word: "thing", f: "2675" },
  { word: "night", f: "2675" },
  { word: "pivot", f: "2674" },
  { word: "hence", f: "2674" },
  { word: "razor", f: "2672" },
  { word: "shaky", f: "2672" },
  { word: "unfit", f: "2672" },
  { word: "parry", f: "2670" },
  { word: "vixen", f: "2669" },
  { word: "toxic", f: "2668" },
  { word: "undue", f: "2668" },
  { word: "theme", f: "2667" },
  { word: "wordy", f: "2666" },
  { word: "black", f: "2666" },
  { word: "dowry", f: "2666" },
  { word: "rowdy", f: "2666" },
  { word: "forum", f: "2666" },
  { word: "robot", f: "2663" },
  { word: "speed", f: "2662" },
  { word: "koala", f: "2662" },
  { word: "waltz", f: "2662" },
  { word: "exile", f: "2660" },
  { word: "tibia", f: "2660" },
  { word: "ditch", f: "2659" },
  { word: "belly", f: "2658" },
  { word: "sheep", f: "2658" },
  { word: "fatal", f: "2657" },
  { word: "augur", f: "2656" },
  { word: "ledge", f: "2656" },
  { word: "mouth", f: "2655" },
  { word: "frown", f: "2653" },
  { word: "pence", f: "2652" },
  { word: "clink", f: "2652" },
  { word: "wring", f: "2651" },
  { word: "smack", f: "2651" },
  { word: "ought", f: "2650" },
  { word: "tough", f: "2650" },
  { word: "stunk", f: "2650" },
  { word: "alibi", f: "2650" },
  { word: "child", f: "2649" },
  { word: "basal", f: "2648" },
  { word: "allow", f: "2647" },
  { word: "joust", f: "2646" },
  { word: "stint", f: "2644" },
  { word: "ghoul", f: "2640" },
  { word: "slunk", f: "2640" },
  { word: "blind", f: "2639" },
  { word: "brink", f: "2636" },
  { word: "amiss", f: "2635" },
  { word: "rigor", f: "2635" },
  { word: "fudge", f: "2634" },
  { word: "gross", f: "2633" },
  { word: "pitch", f: "2633" },
  { word: "botch", f: "2630" },
  { word: "seven", f: "2630" },
  { word: "queer", f: "2628" },
  { word: "lunch", f: "2627" },
  { word: "needy", f: "2626" },
  { word: "bleed", f: "2626" },
  { word: "prick", f: "2624" },
  { word: "doubt", f: "2624" },
  { word: "vomit", f: "2623" },
  { word: "grimy", f: "2622" },
  { word: "donor", f: "2621" },
  { word: "marry", f: "2619" },
  { word: "stout", f: "2619" },
  { word: "snowy", f: "2618" },
  { word: "honor", f: "2617" },
  { word: "awake", f: "2617" },
  { word: "banjo", f: "2616" },
  { word: "flack", f: "2615" },
  { word: "quilt", f: "2615" },
  { word: "seize", f: "2613" },
  { word: "plush", f: "2611" },
  { word: "catty", f: "2610" },
  { word: "spicy", f: "2609" },
  { word: "moldy", f: "2607" },
  { word: "staff", f: "2607" },
  { word: "musty", f: "2606" },
  { word: "mangy", f: "2606" },
  { word: "godly", f: "2602" },
  { word: "floor", f: "2602" },
  { word: "ozone", f: "2602" },
  { word: "gusty", f: "2601" },
  { word: "music", f: "2600" },
  { word: "basis", f: "2600" },
  { word: "bleep", f: "2600" },
  { word: "grill", f: "2600" },
  { word: "penny", f: "2600" },
  { word: "bylaw", f: "2599" },
  { word: "coach", f: "2599" },
  { word: "flail", f: "2599" },
  { word: "occur", f: "2597" },
  { word: "focus", f: "2597" },
  { word: "verge", f: "2596" },
  { word: "bingo", f: "2592" },
  { word: "awful", f: "2590" },
  { word: "drama", f: "2587" },
  { word: "lynch", f: "2585" },
  { word: "quark", f: "2584" },
  { word: "shown", f: "2582" },
  { word: "theft", f: "2581" },
  { word: "pithy", f: "2581" },
  { word: "hound", f: "2578" },
  { word: "sooty", f: "2577" },
  { word: "opium", f: "2575" },
  { word: "gaudy", f: "2575" },
  { word: "think", f: "2574" },
  { word: "await", f: "2574" },
  { word: "catch", f: "2574" },
  { word: "hatch", f: "2574" },
  { word: "frock", f: "2570" },
  { word: "deuce", f: "2570" },
  { word: "fritz", f: "2569" },
  { word: "amaze", f: "2568" },
  { word: "mogul", f: "2567" },
  { word: "crack", f: "2565" },
  { word: "vapid", f: "2563" },
  { word: "flaky", f: "2563" },
  { word: "guild", f: "2561" },
  { word: "weave", f: "2560" },
  { word: "fewer", f: "2557" },
  { word: "pound", f: "2556" },
  { word: "banal", f: "2554" },
  { word: "beget", f: "2554" },
  { word: "stuck", f: "2552" },
  { word: "junto", f: "2552" },
  { word: "tweed", f: "2550" },
  { word: "enemy", f: "2549" },
  { word: "clung", f: "2549" },
  { word: "stump", f: "2548" },
  { word: "ditto", f: "2547" },
  { word: "idiot", f: "2547" },
  { word: "stood", f: "2545" },
  { word: "drunk", f: "2544" },
  { word: "moron", f: "2544" },
  { word: "vinyl", f: "2543" },
  { word: "mania", f: "2541" },
  { word: "sooth", f: "2541" },
  { word: "shoot", f: "2541" },
  { word: "dwell", f: "2540" },
  { word: "brick", f: "2538" },
  { word: "slump", f: "2538" },
  { word: "tenet", f: "2537" },
  { word: "squad", f: "2537" },
  { word: "aging", f: "2536" },
  { word: "again", f: "2536" },
  { word: "wheel", f: "2536" },
  { word: "quash", f: "2533" },
  { word: "young", f: "2532" },
  { word: "pupal", f: "2532" },
  { word: "sixty", f: "2531" },
  { word: "build", f: "2531" },
  { word: "slosh", f: "2531" },
  { word: "would", f: "2528" },
  { word: "champ", f: "2528" },
  { word: "comma", f: "2526" },
  { word: "swamp", f: "2526" },
  { word: "crump", f: "2526" },
  { word: "colon", f: "2525" },
  { word: "blush", f: "2525" },
  { word: "blown", f: "2524" },
  { word: "dimly", f: "2524" },
  { word: "avail", f: "2522" },
  { word: "villa", f: "2522" },
  { word: "stoop", f: "2519" },
  { word: "frill", f: "2519" },
  { word: "dally", f: "2516" },
  { word: "fence", f: "2515" },
  { word: "privy", f: "2515" },
  { word: "fever", f: "2515" },
  { word: "ficus", f: "2514" },
  { word: "qualm", f: "2510" },
  { word: "phony", f: "2510" },
  { word: "spool", f: "2509" },
  { word: "sloop", f: "2509" },
  { word: "cumin", f: "2506" },
  { word: "owing", f: "2506" },
  { word: "fling", f: "2506" },
  { word: "mound", f: "2505" },
  { word: "patty", f: "2500" },
  { word: "shock", f: "2499" },
  { word: "krill", f: "2499" },
  { word: "imply", f: "2498" },
  { word: "delve", f: "2498" },
  { word: "sixth", f: "2495" },
  { word: "swift", f: "2494" },
  { word: "apply", f: "2490" },
  { word: "sulky", f: "2490" },
  { word: "vodka", f: "2489" },
  { word: "truth", f: "2484" },
  { word: "silly", f: "2484" },
  { word: "proxy", f: "2482" },
  { word: "fluid", f: "2480" },
  { word: "pinch", f: "2479" },
  { word: "ionic", f: "2477" },
  { word: "conic", f: "2477" },
  { word: "sense", f: "2477" },
  { word: "thick", f: "2476" },
  { word: "flush", f: "2474" },
  { word: "flown", f: "2473" },
  { word: "nylon", f: "2473" },
  { word: "bound", f: "2470" },
  { word: "adapt", f: "2468" },
  { word: "union", f: "2467" },
  { word: "award", f: "2466" },
  { word: "eject", f: "2466" },
  { word: "excel", f: "2466" },
  { word: "climb", f: "2464" },
  { word: "sweep", f: "2464" },
  { word: "witch", f: "2461" },
  { word: "canny", f: "2456" },
  { word: "cabal", f: "2456" },
  { word: "blink", f: "2456" },
  { word: "parka", f: "2455" },
  { word: "dutch", f: "2455" },
  { word: "igloo", f: "2455" },
  { word: "alpha", f: "2454" },
  { word: "pouch", f: "2454" },
  { word: "gloss", f: "2453" },
  { word: "quell", f: "2448" },
  { word: "wrung", f: "2447" },
  { word: "gipsy", f: "2443" },
  { word: "block", f: "2441" },
  { word: "blitz", f: "2440" },
  { word: "crumb", f: "2440" },
  { word: "sappy", f: "2440" },
  { word: "stunt", f: "2440" },
  { word: "rumor", f: "2436" },
  { word: "dryly", f: "2436" },
  { word: "limit", f: "2435" },
  { word: "gayly", f: "2434" },
  { word: "boost", f: "2433" },
  { word: "showy", f: "2432" },
  { word: "judge", f: "2431" },
  { word: "druid", f: "2430" },
  { word: "venue", f: "2428" },
  { word: "spill", f: "2426" },
  { word: "smock", f: "2426" },
  { word: "naval", f: "2426" },
  { word: "swing", f: "2421" },
  { word: "curvy", f: "2421" },
  { word: "found", f: "2419" },
  { word: "might", f: "2416" },
  { word: "jetty", f: "2414" },
  { word: "batty", f: "2414" },
  { word: "tabby", f: "2414" },
  { word: "droop", f: "2413" },
  { word: "kneed", f: "2411" },
  { word: "axial", f: "2406" },
  { word: "karma", f: "2404" },
  { word: "jelly", f: "2404" },
  { word: "usurp", f: "2402" },
  { word: "cough", f: "2398" },
  { word: "snuck", f: "2398" },
  { word: "roomy", f: "2394" },
  { word: "flock", f: "2390" },
  { word: "bevel", f: "2386" },
  { word: "clack", f: "2385" },
  { word: "gassy", f: "2384" },
  { word: "wound", f: "2384" },
  { word: "fishy", f: "2384" },
  { word: "rugby", f: "2383" },
  { word: "rotor", f: "2382" },
  { word: "beech", f: "2380" },
  { word: "avian", f: "2378" },
  { word: "width", f: "2377" },
  { word: "dingy", f: "2375" },
  { word: "coyly", f: "2375" },
  { word: "dying", f: "2375" },
  { word: "folio", f: "2374" },
  { word: "smoky", f: "2374" },
  { word: "dandy", f: "2372" },
  { word: "floss", f: "2372" },
  { word: "mulch", f: "2368" },
  { word: "quoth", f: "2368" },
  { word: "unify", f: "2368" },
  { word: "salsa", f: "2367" },
  { word: "blurb", f: "2366" },
  { word: "snoop", f: "2365" },
  { word: "spoon", f: "2365" },
  { word: "ninth", f: "2364" },
  { word: "fatty", f: "2363" },
  { word: "taffy", f: "2363" },
  { word: "gulch", f: "2363" },
  { word: "freer", f: "2362" },
  { word: "refer", f: "2362" },
  { word: "filmy", f: "2361" },
  { word: "expel", f: "2356" },
  { word: "blimp", f: "2354" },
  { word: "abyss", f: "2354" },
  { word: "smash", f: "2353" },
  { word: "teeth", f: "2351" },
  { word: "furor", f: "2350" },
  { word: "evoke", f: "2350" },
  { word: "clump", f: "2346" },
  { word: "outdo", f: "2343" },
  { word: "butch", f: "2343" },
  { word: "downy", f: "2342" },
  { word: "spiky", f: "2342" },
  { word: "finch", f: "2342" },
  { word: "milky", f: "2341" },
  { word: "crook", f: "2340" },
  { word: "crock", f: "2340" },
  { word: "bliss", f: "2340" },
  { word: "plunk", f: "2338" },
  { word: "spasm", f: "2331" },
  { word: "fight", f: "2330" },
  { word: "mambo", f: "2330" },
  { word: "tepee", f: "2329" },
  { word: "wispy", f: "2327" },
  { word: "brood", f: "2327" },
  { word: "hedge", f: "2326" },
  { word: "inbox", f: "2318" },
  { word: "murky", f: "2317" },
  { word: "pushy", f: "2317" },
  { word: "dough", f: "2314" },
  { word: "going", f: "2311" },
  { word: "cheek", f: "2309" },
  { word: "check", f: "2309" },
  { word: "booze", f: "2308" },
  { word: "pixie", f: "2308" },
  { word: "winch", f: "2307" },
  { word: "flick", f: "2307" },
  { word: "kiosk", f: "2304" },
  { word: "queen", f: "2304" },
  { word: "fjord", f: "2303" },
  { word: "array", f: "2303" },
  { word: "flung", f: "2302" },
  { word: "toddy", f: "2301" },
  { word: "stiff", f: "2299" },
  { word: "lucky", f: "2298" },
  { word: "wight", f: "2295" },
  { word: "lumpy", f: "2294" },
  { word: "rajah", f: "2294" },
  { word: "icily", f: "2292" },
  { word: "dolly", f: "2291" },
  { word: "oddly", f: "2291" },
  { word: "spunk", f: "2288" },
  { word: "holly", f: "2287" },
  { word: "wacky", f: "2286" },
  { word: "verve", f: "2285" },
  { word: "sully", f: "2280" },
  { word: "groom", f: "2280" },
  { word: "quirk", f: "2276" },
  { word: "punch", f: "2275" },
  { word: "caddy", f: "2274" },
  { word: "rigid", f: "2274" },
  { word: "bezel", f: "2273" },
  { word: "worry", f: "2273" },
  { word: "bawdy", f: "2273" },
  { word: "radar", f: "2271" },
  { word: "skill", f: "2269" },
  { word: "curry", f: "2268" },
  { word: "melee", f: "2268" },
  { word: "thump", f: "2268" },
  { word: "scoop", f: "2267" },
  { word: "mushy", f: "2266" },
  { word: "hitch", f: "2266" },
  { word: "loopy", f: "2265" },
  { word: "polyp", f: "2265" },
  { word: "twist", f: "2264" },
  { word: "pubic", f: "2263" },
  { word: "outgo", f: "2261" },
  { word: "windy", f: "2259" },
  { word: "knoll", f: "2258" },
  { word: "crick", f: "2257" },
  { word: "chili", f: "2256" },
  { word: "chill", f: "2256" },
  { word: "whiny", f: "2255" },
  { word: "fungi", f: "2254" },
  { word: "swill", f: "2254" },
  { word: "ninja", f: "2252" },
  { word: "fauna", f: "2251" },
  { word: "whack", f: "2250" },
  { word: "broom", f: "2250" },
  { word: "proof", f: "2250" },
  { word: "khaki", f: "2249" },
  { word: "uncut", f: "2248" },
  { word: "pinky", f: "2248" },
  { word: "weedy", f: "2246" },
  { word: "slush", f: "2244" },
  { word: "knack", f: "2241" },
  { word: "fugue", f: "2241" },
  { word: "pluck", f: "2240" },
  { word: "vouch", f: "2240" },
  { word: "photo", f: "2239" },
  { word: "wryly", f: "2238" },
  { word: "humid", f: "2236" },
  { word: "skimp", f: "2233" },
  { word: "belle", f: "2233" },
  { word: "pagan", f: "2232" },
  { word: "swash", f: "2232" },
  { word: "awash", f: "2232" },
  { word: "bushy", f: "2231" },
  { word: "savvy", f: "2226" },
  { word: "pupil", f: "2224" },
  { word: "munch", f: "2224" },
  { word: "embed", f: "2223" },
  { word: "visit", f: "2222" },
  { word: "lupus", f: "2222" },
  { word: "ditty", f: "2218" },
  { word: "comic", f: "2218" },
  { word: "swung", f: "2217" },
  { word: "lymph", f: "2216" },
  { word: "geese", f: "2213" },
  { word: "shuck", f: "2212" },
  { word: "glyph", f: "2211" },
  { word: "cocoa", f: "2210" },
  { word: "cacao", f: "2210" },
  { word: "golly", f: "2209" },
  { word: "fanny", f: "2209" },
  { word: "dilly", f: "2208" },
  { word: "idyll", f: "2208" },
  { word: "hilly", f: "2204" },
  { word: "comfy", f: "2202" },
  { word: "bough", f: "2202" },
  { word: "shyly", f: "2202" },
  { word: "flunk", f: "2201" },
  { word: "condo", f: "2199" },
  { word: "nutty", f: "2196" },
  { word: "sushi", f: "2196" },
  { word: "mafia", f: "2196" },
  { word: "conch", f: "2195" },
  { word: "forgo", f: "2194" },
  { word: "swoon", f: "2193" },
  { word: "bunch", f: "2189" },
  { word: "booty", f: "2189" },
  { word: "ruddy", f: "2184" },
  { word: "thumb", f: "2182" },
  { word: "hippo", f: "2181" },
  { word: "manga", f: "2181" },
  { word: "hurry", f: "2180" },
  { word: "geeky", f: "2179" },
  { word: "lobby", f: "2179" },
  { word: "penne", f: "2175" },
  { word: "jewel", f: "2174" },
  { word: "beefy", f: "2169" },
  { word: "paddy", f: "2164" },
  { word: "mossy", f: "2164" },
  { word: "dusky", f: "2164" },
  { word: "quack", f: "2162" },
  { word: "cabby", f: "2162" },
  { word: "husky", f: "2160" },
  { word: "happy", f: "2160" },
  { word: "clock", f: "2160" },
  { word: "soggy", f: "2159" },
  { word: "tweet", f: "2157" },
  { word: "howdy", f: "2156" },
  { word: "booth", f: "2153" },
  { word: "duchy", f: "2151" },
  { word: "lipid", f: "2150" },
  { word: "picky", f: "2150" },
  { word: "plumb", f: "2150" },
  { word: "cynic", f: "2148" },
  { word: "juror", f: "2147" },
  { word: "blood", f: "2147" },
  { word: "sniff", f: "2145" },
  { word: "brook", f: "2144" },
  { word: "sunny", f: "2136" },
  { word: "vying", f: "2135" },
  { word: "idiom", f: "2134" },
  { word: "whisk", f: "2134" },
  { word: "tatty", f: "2133" },
  { word: "wedge", f: "2132" },
  { word: "scoff", f: "2130" },
  { word: "gumbo", f: "2129" },
  { word: "bossy", f: "2129" },
  { word: "folly", f: "2128" },
  { word: "allay", f: "2123" },
  { word: "unzip", f: "2120" },
  { word: "gawky", f: "2120" },
  { word: "chunk", f: "2118" },
  { word: "squib", f: "2117" },
  { word: "cinch", f: "2112" },
  { word: "timid", f: "2109" },
  { word: "undid", f: "2106" },
  { word: "bitty", f: "2106" },
  { word: "level", f: "2105" },
  { word: "plaza", f: "2105" },
  { word: "digit", f: "2104" },
  { word: "bulky", f: "2102" },
  { word: "gloom", f: "2100" },
  { word: "tight", f: "2100" },
  { word: "thigh", f: "2100" },
  { word: "cliff", f: "2097" },
  { word: "flood", f: "2096" },
  { word: "billy", f: "2096" },
  { word: "stuff", f: "2095" },
  { word: "wooly", f: "2093" },
  { word: "lowly", f: "2093" },
  { word: "couch", f: "2087" },
  { word: "musky", f: "2087" },
  { word: "missy", f: "2081" },
  { word: "click", f: "2077" },
  { word: "chaff", f: "2075" },
  { word: "sassy", f: "2073" },
  { word: "assay", f: "2073" },
  { word: "nymph", f: "2072" },
  { word: "bloom", f: "2070" },
  { word: "juicy", f: "2067" },
  { word: "hunky", f: "2066" },
  { word: "papal", f: "2065" },
  { word: "skull", f: "2065" },
  { word: "skulk", f: "2065" },
  { word: "hutch", f: "2062" },
  { word: "pizza", f: "2057" },
  { word: "fifty", f: "2055" },
  { word: "filly", f: "2045" },
  { word: "kitty", f: "2035" },
  { word: "icing", f: "2034" },
  { word: "myrrh", f: "2029" },
  { word: "emcee", f: "2026" },
  { word: "shook", f: "2022" },
  { word: "furry", f: "2021" },
  { word: "spoof", f: "2020" },
  { word: "witty", f: "2020" },
  { word: "civil", f: "2020" },
  { word: "bosom", f: "2020" },
  { word: "fifth", f: "2019" },
  { word: "chump", f: "2016" },
  { word: "knock", f: "2016" },
  { word: "llama", f: "2014" },
  { word: "widow", f: "2013" },
  { word: "willy", f: "2010" },
  { word: "dully", f: "2004" },
  { word: "maxim", f: "2003" },
  { word: "spook", f: "2000" },
  { word: "onion", f: "2000" },
  { word: "baggy", f: "1996" },
  { word: "putty", f: "1988" },
  { word: "pooch", f: "1987" },
  { word: "swoop", f: "1985" },
  { word: "nanny", f: "1979" },
  { word: "pulpy", f: "1978" },
  { word: "wimpy", f: "1974" },
  { word: "ovoid", f: "1971" },
  { word: "dumpy", f: "1968" },
  { word: "macaw", f: "1967" },
  { word: "amass", f: "1964" },
  { word: "pudgy", f: "1963" },
  { word: "hussy", f: "1950" },
  { word: "aback", f: "1947" },
  { word: "snuff", f: "1941" },
  { word: "livid", f: "1936" },
  { word: "mucus", f: "1929" },
  { word: "jolly", f: "1925" },
  { word: "swish", f: "1924" },
  { word: "gully", f: "1922" },
  { word: "bongo", f: "1921" },
  { word: "skunk", f: "1921" },
  { word: "affix", f: "1917" },
  { word: "guava", f: "1910" },
  { word: "hunch", f: "1908" },
  { word: "gruff", f: "1907" },
  { word: "funky", f: "1907" },
  { word: "cubic", f: "1896" },
  { word: "mucky", f: "1895" },
  { word: "bully", f: "1892" },
  { word: "moody", f: "1888" },
  { word: "quill", f: "1886" },
  { word: "goody", f: "1883" },
  { word: "dodgy", f: "1883" },
  { word: "kinky", f: "1881" },
  { word: "cluck", f: "1873" },
  { word: "tooth", f: "1872" },
  { word: "plump", f: "1869" },
  { word: "buxom", f: "1855" },
  { word: "quick", f: "1854" },
  { word: "vigil", f: "1854" },
  { word: "hippy", f: "1852" },
  { word: "hobby", f: "1849" },
  { word: "jumbo", f: "1845" },
  { word: "humus", f: "1841" },
  { word: "fully", f: "1841" },
  { word: "chock", f: "1830" },
  { word: "slyly", f: "1813" },
  { word: "giddy", f: "1800" },
  { word: "motto", f: "1799" },
  { word: "daddy", f: "1797" },
  { word: "fussy", f: "1791" },
  { word: "skiff", f: "1780" },
  { word: "femme", f: "1779" },
  { word: "piggy", f: "1774" },
  { word: "gypsy", f: "1772" },
  { word: "biddy", f: "1770" },
  { word: "woody", f: "1767" },
  { word: "dowdy", f: "1767" },
  { word: "sissy", f: "1765" },
  { word: "bunny", f: "1748" },
  { word: "chick", f: "1747" },
  { word: "known", f: "1734" },
  { word: "which", f: "1732" },
  { word: "queue", f: "1729" },
  { word: "foggy", f: "1720" },
  { word: "goofy", f: "1720" },
  { word: "mammy", f: "1720" },
  { word: "whoop", f: "1705" },
  { word: "funny", f: "1697" },
  { word: "bluff", f: "1697" },
  { word: "madam", f: "1688" },
  { word: "ninny", f: "1671" },
  { word: "twixt", f: "1632" },
  { word: "kayak", f: "1614" },
  { word: "gamma", f: "1606" },
  { word: "magma", f: "1606" },
  { word: "jumpy", f: "1602" },
  { word: "dummy", f: "1601" },
  { word: "muddy", f: "1601" },
  { word: "guppy", f: "1570" },
  { word: "buddy", f: "1566" },
  { word: "minim", f: "1562" },
  { word: "kappa", f: "1556" },
  { word: "poppy", f: "1546" },
  { word: "chuck", f: "1543" },
  { word: "humph", f: "1539" },
  { word: "dizzy", f: "1529" },
  { word: "shush", f: "1525" },
  { word: "gummy", f: "1519" },
  { word: "boozy", f: "1500" },
  { word: "puffy", f: "1489" },
  { word: "whiff", f: "1485" },
  { word: "buggy", f: "1484" },
  { word: "jazzy", f: "1471" },
  { word: "mimic", f: "1464" },
  { word: "booby", f: "1460" },
  { word: "bobby", f: "1460" },
  { word: "pygmy", f: "1419" },
  { word: "fluff", f: "1416" },
  { word: "woozy", f: "1414" },
  { word: "fizzy", f: "1366" },
  { word: "jiffy", f: "1353" },
  { word: "civic", f: "1301" },
  { word: "mamma", f: "1295" },
  { word: "puppy", f: "1259" },
  { word: "vivid", f: "1217" },
  { word: "mummy", f: "1208" },
  { word: "fuzzy", f: "1162" },
];

col = 0;
row = 0;

// display the results list on page load
window.addEventListener("load", (event) => {
  findGuesses();
});

// listener for board text input, handles typing input on the board
document.addEventListener("keydown", function (event) {
  if (event.key == "Backspace") {
    // erase last letter
    if (col <= 0 && row > 0) {
      // retreat col/row
      col = 5;
      if (row > 0) row--;
    }
    if (col > 0) col--;
    document.getElementById("board").children[row].children[col].textContent =
      " ";
    document
      .getElementById("board")
      .children[row].children[col].setAttribute("colour", "none");
    findGuesses();
  }
  if (!/^[a-z]$/i.test(event.key)) return; // only accept letters as input
  if (row == 5 && col == 5) return; // board is full cant input
  document.getElementById("board").children[row].children[col].textContent =
    event.key;
  document
    .getElementById("board")
    .children[row].children[col].setAttribute("colour", "grey");
  if (col <= 4) col++; // advance col/row
  if (col >= 5 && row < 5) {
    col = 0;
    row++;
  }
  findGuesses();
});

// listener for board click input, handles mouse clicks that change the colour of tiles
document.addEventListener(
  "click",
  function (e) {
    e = e || window.event;
    if (e.target.textContent == " ") return; // empty tile, nothing to update
    if (e.target.getAttribute("colour") == "grey") {
      // grey tile, turns yellow
      e.target.setAttribute("colour", "yellow");
    } else if (e.target.getAttribute("colour") == "yellow") {
      // yellow tile, turns green
      e.target.setAttribute("colour", "green");
    } else if (e.target.getAttribute("colour") == "green") {
      // green tile, turns grey
      e.target.setAttribute("colour", "grey");
    }
    findGuesses();
  },
  false
);

// check the current guess contains all green letter in the correct positions
function findGuesses() {
  // guess letters
  guesses = []; // words
  green = []; // letter and position correct
  yellow = [[],[],[],[],[]]; // letter correct position wrong
  grey = []; // letter not in answer

  // examine the board
  tiles = document.getElementsByClassName("tile");
  for (let i = 0; i < tiles.length; i++) {
    if (tiles[i].textContent != " ") {
      // check tile type, update lists
      if (tiles[i].getAttribute("colour") == "green") {
        green[tiles[i].getAttribute("col")] = tiles[i].textContent;
      } else if (tiles[i].getAttribute("colour") == "yellow") {
        yellow[tiles[i].getAttribute("col")].push(tiles[i].textContent);
      } else if (tiles[i].getAttribute("colour") == "grey") {
        grey.push(tiles[i].textContent);
      }
    }
  }
  i = 0; // check for the most likely 15 words
  while (guesses.length < 15 && i < words.length) {
    if (
      checkGreen(green, words[i].word) &&
      checkYellow(yellow, words[i].word) &&
      checkGrey(grey, words[i].word)
    ) {
      guesses.push(words[i].word);
    }
    i++;
  }
  // update results list
  var resultsList = document.getElementById("results");
  resultsList.innerHTML = "";
  for (let i = 0; i < guesses.length; i++) {
    resultsList.innerHTML += "<li>" + guesses[i] + "</li>";
  }
}

// check the current guess contains all green letter in the correct positions
function checkGreen(greenLetters, word) {
  for (let i = 0; i < greenLetters.length; i++) {
    if (greenLetters[i] != null && greenLetters[i] != word[i]) {
      return false;
    }
  }
  return true;
}

// check the current guess contains all yellow letters, not in the positions they were guessed.
function checkYellow(yellowLetters, word) {
  for (let i = 0; i < yellowLetters.length; i++) {
    for (let j = 0; j < yellowLetters[i].length; j++) {
      if (!word.includes(yellowLetters[i][j]) || yellowLetters[i][j] == word[i]) {
        return false;
      }
    }
  }
  return true;
}

// check the current guess contains no grey letters
function checkGrey(greyLetters, word) {
  for (let i = 0; i < greyLetters.length; i++) {
    if (word.includes(greyLetters[i])) {
      return false;
    }
  }
  return true;
}
